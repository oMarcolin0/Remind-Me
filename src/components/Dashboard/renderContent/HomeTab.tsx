...
import { Pill, PlusCircle, Clock, CheckCircle, MessageCircle } from 'lucide-react';
...
interface HomeTabProps {
  ...
  handleToggleComplete: (id: string, currentValue: boolean) => void;
            ...
            medicines.map((medicine) => (
              <motion.div
                ...
                className={`p-4 rounded-lg border transition-all hover:shadow-md ${
                  medicine.completed ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <button
                      onClick={() => handleToggleComplete(medicine.id, medicine.completed)}
                      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center ${
                        medicine.completed ? 'bg-green-500 border-green-500' : 'border-[#20B2AA]'
                      }`}
                      aria-label={medicine.completed ? 'Marcar como pendente' : 'Marcar como tomado'}
                    >
                      {medicine.completed && <CheckCircle size={14} className="text-white" />}
                    </button>
                    <div>
                      <h3 className={`font-semibold ${medicine.completed ? 'text-gray-500 line-through' : 'text-gray-800'}`}>
                        {medicine.name}
                      </h3>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex gap-1 flex-wrap">
                      {medicine.time?.split(',').map((t, i) => (
                        <span
                          key={i}
                          className={`px-2 py-1 rounded text-sm font-medium ${
                            medicine.completed ? 'bg-gray-100 text-gray-500' : 'bg-[#20B2AA]/10 text-[#20B2AA]'
                          }`}
                        >
                          {t.trim()}
                        </span>
